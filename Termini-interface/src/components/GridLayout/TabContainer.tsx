...
export default function TabContainer({ 
  ...
  const handleTabDragStart = (e: React.MouseEvent, pluginId: string, index: number) => {
    ...
    if (onDragTab && tabId) {
      // 存储拖拽起始位置（用于计算偏移量）
      const tabElement = e.currentTarget.getBoundingClientRect();
      (window as any).__draggedPluginPosition = {
        x: e.clientX - tabElement.left,
        y: e.clientY - tabElement.top,
        w: tabElement.width,
        h: tabElement.height
      };
          ...
          if (dragInfo) {
            try {
              ...
              onSplitTab(draggingTabId, {
                x: Math.floor(upEvent.clientX / (dragInfo.w / 6)), // 转换为网格坐标
                y: Math.floor(upEvent.clientY / (dragInfo.h / 6)),
                w: 6, // 默认宽度
                h: 6  // 默认高度
              });
            } catch (err) {
