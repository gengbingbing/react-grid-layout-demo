...
export function RatingToast({ onRatingClick }: Props) {
  return (
    <div>
      ...
      <div className="mb-6 flex w-full flex-row justify-between" style={{ color: "var(--color-text-secondary)" }}>
        <div>
...
function RatingItem({ rating, onClick }: { rating: number; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      className="flex h-26 w-26 cursor-pointer select-none items-center justify-center text-center align-middle hover:bg-[var(--color-hover)]"
      style={{ 
