...
type InnerTooltipProps<T extends ElementType | undefined> = {
  ...
  as?: T;
  /**
   * Whether to render the tooltip in a portal. Defaults to true.
   * This helps prevent the tooltip from being clipped by parent elements.
   */
  withPortal?: boolean;
...
export default function Tooltip<T extends ElementType>({
  ...
  const tooltipContent = visible ? (
    ...
    >
      <FloatingArrow ref={arrowRef} context={context} className="fill-[var(--color-bg-tips)]" />
      {finalContent}
    </div>
  ) : undefined;
