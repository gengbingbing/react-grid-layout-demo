...
import { CHAIN_ID, getExplorerUrl,MINATO_MAINNET } from "config/chains";
import { isLocal, REACT_APP_HOST_URL } from "config/env";
...
export function getPageTitle(data) {
  const title = t`Decentralized Perpetual Exchange | Termini`;
  return `${data} | ${title}`;
}
