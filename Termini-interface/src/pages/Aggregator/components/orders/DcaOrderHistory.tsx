...
export default function DcaOrderHistory(props) {
  const { tokenMap, tradeTranTypeTab, setTradeTranTypeTab, setPendingTxns, loading, hasNewOrder, setHasNewOrder } = props;
  ...
  }, [tradeTranTypeTab, orderDcaHistoryData]);
  ...
  const renderActionButton = useCallback((order: any) => {
    ...
    return (
      <div className="flex items-center justify-between text-[var(--color-primary)] text-[var(--fontSize-sm)] cursor-pointer">
        ...
        >
          {expandedInfo[order.id] ? <FiChevronDown size={20} color="var(--color-text-tertiary)" /> : <FiChevronRight size={20} color="var(--color-text-tertiary)" />}
        </div>
      </div>
    );
  }, [chainId, expandedInfo, handleToggleExpand]);
  ...
  const renderOrderRow = useCallback((order: any, index: number) => {
    ...
    const totalDeposited = formatNumberKM(formatAmount(fromTokenVal, parseInt(tokenIn.decimals as any), 4, false));
  ...
  return (
    <div className="order-list-table pb-[10px]">
      <div className={cx("table-wrapper relative px-[var(--spacing-3)]", {
        "pb-[5rem]": currentPageData?.length
      })}>
        <table className="token--table token--table-dca">
          ...
          <tbody>
            {currentPageData?.map(renderOrderRow)}
          </tbody>
        </table>
        ...
        {pageCount > 1 && currentPageData?.length > 0 && (
          <Pagination
            page={currentPage}
            pageCount={pageCount}
