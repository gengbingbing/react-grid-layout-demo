  ...
  const onDragStart = (layout: Layout[], oldItem: Layout, newItem: Layout) => {
    ...
    gridItems.forEach(item => {
      if (item instanceof HTMLElement) {
        ...
        if (targetId) {
          // 如果是标签容器，添加特殊提示样式
          if (targetId.startsWith('tab-container-') && !itemId.startsWith('tab-container-')) {
            item.classList.add('tab-container-droppable');
            // 添加浅色边框提示
            item.style.outline = '2px dashed rgba(39, 174, 96, 0.4)';
      ...
      if (container instanceof HTMLElement) {
        ...
        if (targetTabId && targetTabId !== tabId) {
          ...
          if (
            ...
            try {
              // 调用移动插件的函数
              createTabContainer(pluginId, targetTabId);
              targetFound = true;
