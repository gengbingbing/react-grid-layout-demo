  ...
  persist(
    (set, get) => ({
      ...
      removePluginFromTab: (tabId: string, pluginId: string, customPosition?: {x: number, y: number, w: number, h: number}) => set((state) => {
        ...
        // 1. 新增限制: 如果标签容器只有一个插件，不允许将其拖出
        if (tabContainer.plugins.length <= 1) {
          console.log('标签容器只有一个插件，不允许拖出，只能移动整个容器');
          return state;
        }
        
        // 检查插件是否已经在布局中存在（防止重复）
        const existingLayout = state.layout.find(item => item.i === pluginId);
        if (existingLayout) {
          console.warn('插件已存在于布局中，移除它:', pluginId);
          // 确保从布局中先移除该插件
          const updatedLayout = state.layout.filter(item => item.i !== pluginId);
          ...
          // 如果标签容器为空，需要从布局中移除
          const finalLayout = updatedLayout.filter(item => {
            if (item.i.startsWith('tab-container-')) {
              return updatedTabContainers.some(tab => tab.id === item.i);
            }
            return true;
          });
          ...
          return {
            ...
            layout: finalLayout,
            tabContainers: updatedTabContainers,
            // 仍然保留插件在activePlugins中，只需要确保它在正确的标签容器内
        ...
        const updatedTabContainers = state.tabContainers.map(tab => {
          ...
          return tab;
        }).filter(tab => tab.plugins.length > 0); // 仅保留有插件的标签容器
        
        // 为移出的插件创建新的标签容器，而不是直接添加为独立插件
        const newTabId = `tab-container-${Date.now()}`;
        
        // 获取标签容器的位置信息作为参考
        const tabLayout = state.layout.find(item => item.i === tabId);
        
        let newTabLayout: Layout;
        if (customPosition) {
          // 使用自定义位置
          newTabLayout = {
            i: newTabId,
          ...
          console.log('使用自定义位置添加新标签容器:', newTabLayout);
        } else if (tabLayout) {
          // 使用标签容器的位置并添加偏移
          newTabLayout = {
            i: newTabId,
            x: (tabLayout.x + 1) % 12, // 轻微偏移
            y: tabLayout.y,
            w: Math.min(tabLayout.w, 6), // 不超过6个单位宽
            h: Math.min(tabLayout.h, 6), // 不超过6个单位高
            minW: 3,
            minH: 4
          };
          console.log('使用标签容器位置添加新标签容器:', newTabLayout);
        } else {
          // 使用默认位置
          newTabLayout = {
            i: newTabId,
          ...
          console.log('使用默认位置添加新标签容器:', newTabLayout);
        }
        ...
        // 更新layout，移除空标签容器
        const updatedLayout = state.layout.filter(item => {
          if (item.i.startsWith('tab-container-')) {
            return updatedTabContainers.some(tab => tab.id === item.i);
          }
          return true;
        ...
        // 保留插件在activePlugins中，确保系统能正确识别
        return {
          ...state,
          layout: [...updatedLayout, newTabLayout],
          tabContainers: [...updatedTabContainers, newTabContainer],
          activePlugins: state.activePlugins // 保留原activePlugins不变
        };
      }),
